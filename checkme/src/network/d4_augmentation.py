...
def _recompute_valid_7x7_from_plane(occ_plane: np.ndarray) -> np.ndarray:
    """Recompute valid_7x7 from a D4-transformed (9,9) occupancy plane (1=filled, 0=empty).

    valid_7x7 is a per-region feature (not per-cell), so plain spatial
    rotation gives wrong results. This recomputes it from the transformed board.
    """
...
def transform_state(
    ...
    # valid_7x7 (ch 6-7) is a per-region feature; recompute from transformed boards
    if ti != 0:
