...
class Evaluator:
    ...
    def evaluate_vs_checkpoint(
        ...
        Args:
            ...
            sprt_cfg: Optional SPRT early-stopping config (see evaluate_vs_baseline).
    ...
    def evaluate_vs_baseline(
        ...
        sprt_cfg: optional dict enabling SPRT early-stopping.  Keys:
            p0 (float)      null win rate           default 0.50
            p1 (float)      alternative win rate    default 0.55
            alpha (float)   false-positive rate     default 0.05
            beta  (float)   false-negative rate     default 0.05
            min_games (int) always play at least N  default 60
          SPRT stops early when LLR crosses a bound or when neither bound is
          reachable with remaining games (futility).  Elo is always computed
          from the observed win rate regardless of the decision.
        """
        ...
        logger.debug(f"Evaluating {candidate_name} vs {baseline_label}")

        if int(num_games) <= 0:
            logger.debug("num_games <= 0; skipping evaluation.")
            return {
                "model_wins": 0, "baseline_wins": 0, "ties": 0, "total_games": 0,
                "win_rate": 0.0, "avg_game_length": 0.0, "avg_score_diff": 0.0,
                "avg_model_score_margin": 0.0, "avg_win_margin": 0.0,
                "avg_loss_margin": 0.0, "results": [],
            }
        ...
        eval_sims  = int(eval_config["simulations"])
        eval_cpuct = float(eval_config["cpuct"])

        model_mcts = create_optimized_mcts(
            model, self.config, self.device, self.state_encoder, self.action_encoder,
            eval_mode=True,
        )
        model_mcts.config.simulations = eval_sims
        model_mcts.config.cpuct = eval_cpuct
        ...
        if baseline_type == "pure_mcts":
            ...
            pure_mcts_evaluator = PureMCTSEvaluator(
                simulations=baseline_sims, exploration=1.4, seed=self.config.get("seed", 42),
            )
            ...
            baseline_state_dict = get_state_dict_for_inference(
                baseline_checkpoint, self.config, for_selfplay=False
            )
            load_model_checkpoint(baseline_model, baseline_state_dict)
            baseline_model.to(self.device).eval()
            baseline_mcts = create_optimized_mcts(
                baseline_model, self.config, self.device, self.state_encoder, self.action_encoder,
                eval_mode=True,
            )
            baseline_mcts.config.simulations = eval_sims
            baseline_mcts.config.cpuct = eval_cpuct
        ...
        # ── SPRT setup ─────────────────────────────────────────────────────
        _sprt_active = sprt_cfg is not None
        if _sprt_active:
            _p0       = float(sprt_cfg.get("p0",       0.50))
            _p1       = float(sprt_cfg.get("p1",       0.55))
            _alpha    = float(sprt_cfg.get("alpha",    0.05))
            _beta     = float(sprt_cfg.get("beta",     0.05))
            _sprt_min = int(sprt_cfg.get("min_games",   60))
            _sprt_lower, _sprt_upper = sprt_bounds(_alpha, _beta)
            _win_inc  = sprt_llr_update(True,  _p0, _p1)
            _loss_inc = sprt_llr_update(False, _p0, _p1)
            logger.debug(
                f"SPRT: H0<={_p0}  H1>={_p1}  alpha={_alpha}  beta={_beta}  "
                f"min={_sprt_min}  bounds=[{_sprt_lower:.3f}, {_sprt_upper:.3f}]"
            )
        else:
            _p0 = _p1 = _alpha = _beta = _win_inc = _loss_inc = 0.0
            _sprt_min = num_games
            _sprt_lower = _sprt_upper = 0.0
        _llr = 0.0
        _sprt_done = False
        ...
        schedule    = build_eval_schedule(
            int(num_games), base_seed, paired_eval, game_offset=int(game_offset)
        )
            ...
            for game_idx, (seed, model_plays_first) in enumerate(schedule):
                ...
                try:
                    result = self._play_match(
                        model_mcts, baseline_mcts, pure_mcts_evaluator, model_plays_first, seed
                    )
                    if paired_eval:
                        result["pair_seed"] = seed
                    results.append(result)
                    _wins_cnt += int(result["model_won"])
                except Exception as e:
                    logger.error(f"Eval game {game_idx} error: {e}")
                    continue

                n_done = len(results)
                wr     = _wins_cnt / n_done

                if progress_callback and n_done % progress_interval == 0:
                    progress_callback(n_done, _wins_cnt, wr)
                ...
                if _sprt_active:
                    ...
                    if n_done >= _sprt_min:
                        ...
                        worst_llr = _llr + remaining * _loss_inc
                        if _llr >= _sprt_upper:
                            _sprt_decision = "accept"
                            _sprt_done = True
                            logger.debug(f"SPRT ACCEPT at game {n_done}: WR={wr:.3f}  LLR={_llr:.3f}")
                        elif _llr <= _sprt_lower:
                            _sprt_decision = "reject"
                            _sprt_done = True
                            logger.debug(f"SPRT REJECT at game {n_done}: WR={wr:.3f}  LLR={_llr:.3f}")
                        elif best_llr < _sprt_upper and worst_llr > _sprt_lower:
                            _sprt_decision = "inconclusive"
                            _sprt_done = True
                            logger.debug(
                                f"SPRT INCONCLUSIVE at game {n_done}: neither bound reachable  "
                                f"LLR={_llr:.3f}  possible=[{worst_llr:.3f}, {best_llr:.3f}]"
                            )
                        elif best_llr < _sprt_upper:
                            _sprt_decision = "reject"
                            _sprt_done = True
                            logger.debug(
                                f"SPRT FUTILITY REJECT at game {n_done}: "
                                f"best_possible={best_llr:.3f} < upper={_sprt_upper:.3f}"
                            )
        ...
        if _sprt_active:
            stats["sprt"] = {
                "decision":    _sprt_decision,
                "llr":         round(_llr, 4),
                "lower_bound": round(_sprt_lower, 4),
                "upper_bound": round(_sprt_upper, 4),
                "p0": _p0, "p1": _p1,
                "games_played": len(results),
            }
            logger.info(
                "SPRT %s  games=%d  WR=%.3f  LLR=%.3f",
                _sprt_decision.upper(), len(results), stats["win_rate"], _llr,
            )

        logger.debug(
            f"Results: wins={stats['model_wins']}  losses={stats['baseline_wins']}  "
        ...
        if baseline_model is not None:
            del baseline_model
        if baseline_mcts is not None and baseline_type != "anchor":
            del baseline_mcts
    ...
    def evaluate_sprt(
        ...
        logger.debug(
            "SPRT config: H0≤%.3f  H1≥%.3f  α=%s  β=%s  bounds=[%.3f, %.3f]  games=[%d, %d]",
            p0, p1, alpha, beta, lower, upper, min_games, max_games,
        )
        ...
        _loss_inc = sprt_llr_update(False, p0, p1)
            ...
            for game_idx, (seed, model_plays_first) in enumerate(schedule):
                ...
                try:
                    ...
                    if games_so_far >= min_games:
                        if llr >= upper:
                            logger.info(
                                "SPRT ACCEPT  game=%d  WR=%.3f  LLR=%.3f",
                                games_so_far, wr, llr,
                            )
                        ...
                        if llr <= lower:
                            logger.info(
                                "SPRT REJECT  game=%d  WR=%.3f  LLR=%.3f",
                                games_so_far, wr, llr,
                            )
                        ...
                        if remaining > 0:
                            best_possible_llr  = llr + remaining * _win_inc
                            worst_possible_llr = llr + remaining * _loss_inc

                            if best_possible_llr < upper and worst_possible_llr > lower:
                                # Neither bound reachable → guaranteed inconclusive
                                logger.info(
                                    "SPRT EARLY STOP  game=%d  WR=%.3f  LLR=%.3f  (neither bound reachable)",
                                    games_so_far, wr, llr,
                                )
                                ...
                                logger.info(
                                    "SPRT EARLY REJECT  game=%d  WR=%.3f  LLR=%.3f  (futility — cannot reach ACCEPT)",
                                    games_so_far, wr, llr,
                                )
                                sprt_result.reject = True
                                break
                    ...
                    logger.error("Error in SPRT game %d: %s", game_idx, e)
                    continue
        ...
        if not sprt_result.accept and not sprt_result.reject:
            ...
            wr = _wins_cnt / max(1, len(results))
            logger.info(
                "SPRT INCONCLUSIVE  games=%d  WR=%.3f  LLR=%.3f",
                len(results), wr, llr,
            )

        sprt_result.llr = llr
        sprt_result.games_played = len(results)
        sprt_result.wins = _wins_cnt
        sprt_result.losses = sprt_result.games_played - _wins_cnt
        sprt_result.win_rate = _wins_cnt / max(1, sprt_result.games_played)
...
def compute_implied_elo(win_rate: float) -> float:
    """Compute the Elo gap implied by an observed win rate (400-scale logistic).

    Returns positive value when win_rate > 0.5 (candidate is stronger).
    Clamps to [0.01, 0.99] to avoid ±infinity at extremes.
    """
    wr = max(0.01, min(0.99, float(win_rate)))
    return 400.0 * math.log10(wr / (1.0 - wr))


class LadderEloTracker:
    """Rolling implied-Elo ladder for AlphaZero training progress tracking.

    Every `lookback` iterations, 100 games are played between iter_N and
    iter_(N-lookback). The observed win rate converts directly to an implied
    Elo gap via the 400-scale logistic formula. Cumulative Elo starts at
    base_elo (iter 0) and grows by the implied gap at each ladder step.

    This gives a clean per-step improvement signal without K-factor tuning:
      - WR 55% → +35 Elo   (moderate, LR still doing work)
      - WR 57% → +50 Elo   (strong, delay LR drop)
      - WR 53% → +18 Elo   (weak, LR approaching done)
      - WR 50% → +0  Elo   (plateau)
    """

    def __init__(self, lookback: int = 5, base_elo: float = 1500.0):
        self.lookback = int(lookback)
        self.base_elo = float(base_elo)
        self.entries: List[Dict] = []

    def has_entry(self, iter_n: int) -> bool:
        return any(e["iter"] == iter_n for e in self.entries)

    def add_result(self, iter_n: int, wins: int, losses: int) -> Dict:
        """Record a match result for iter_N vs iter_(N-lookback).

        Returns the new entry dict with implied_gap and cumulative_elo.
        """
        total = wins + losses
        win_rate = wins / total if total > 0 else 0.5
        implied_gap = compute_implied_elo(win_rate)
        prev_elo = self.entries[-1]["cumulative_elo"] if self.entries else self.base_elo
        cumulative_elo = prev_elo + implied_gap
        entry = {
            "iter": int(iter_n),
            "vs_iter": int(iter_n) - self.lookback,
            "wins": int(wins),
            "losses": int(losses),
            "total": int(total),
            "win_rate": round(float(win_rate), 4),
            "implied_gap": round(float(implied_gap), 1),
            "cumulative_elo": round(float(cumulative_elo), 1),
        }
        self.entries.append(entry)
        return entry

    def get_cumulative_elo(self, iter_n: int) -> Optional[float]:
        for e in self.entries:
            if e["iter"] == iter_n:
                return float(e["cumulative_elo"])
        return None

    def latest_step_gap(self) -> Optional[float]:
        return float(self.entries[-1]["implied_gap"]) if self.entries else None

    def format_table(self) -> str:
        lines = ["[LADDER ELO TABLE]"]
        lines.append(f"  {'Iter':>5}  {'vs':>5}  {'W':>4}  {'L':>4}  {'WR':>6}  {'Gap':>7}  {'Cumul Elo':>10}")
        lines.append("  " + "-" * 52)
        for e in self.entries:
            lines.append(
                f"  {e['iter']:>5}  {e['vs_iter']:>5}  {e['wins']:>4}  {e['losses']:>4}  "
                f"{e['win_rate']*100:>5.1f}%  {e['implied_gap']:>+7.1f}  {e['cumulative_elo']:>10.1f}"
            )
        return "\n".join(lines)
    ...
    def load(self, path: Path) -> bool:
        ...
        try:
            with open(path, "r", encoding="utf-8") as f:
                state = json.load(f)
            self.lookback = int(state.get("lookback", self.lookback))
            self.base_elo = float(state.get("base_elo", self.base_elo))
            self.entries = state.get("entries", [])
            return True
        except Exception as e:
            logger.warning("Failed to load ladder Elo from %s: %s", path, e)
            return False
